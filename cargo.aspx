<%@ Page Title="" Language="C#" MasterPageFile="main.Master" AutoEventWireup="true" CodeBehind="cargo.aspx.cs" Inherits="esmeraldino.site.cargo" %>
	...
	<%
		...
		if (Request.QueryString.Count > 0)
		{
			...
			if (comandogeral == "edit")
			{
				...
				connin.Open();
				sqlconsulta.CommandText = "Select DESCRICAO from CARGOS where ID=" + sCODIGO;
		...
		if (Request.Form.Count> 0)
		{
			...
			if ((comando == "SAVE") &&(comandogeral=="new"))
			{
				...
				connin.Open();
				sqlinsert.CommandText = "insert into CARGOS (DESCRICAO) values ('" + pDESCRICAO + "')";
				int result = sqlinsert.ExecuteNonQuery();
				connin.Close();
				Response.Redirect("cargoconsulta.aspx");
			}
			...
			if ((comando == "SAVE") &&(comandogeral=="edit"))
			{
				...
				sqlinsert.Connection = connin;
				connin.Open();
				sqlinsert.CommandText = "update CARGOS set DESCRICAO='" + pDESCRICAO + "' where ID="+sCODIGO;
				int result = sqlinsert.ExecuteNonQuery();
				connin.Close();
				Response.Redirect("cargoconsulta.aspx");
			}
		...
		%>
		<!--Tem que puxar o bootstap aqui, mas oq isso faz?-->
